library(ggplot2)
library(grid)
annotation_table<-read.table("FP2_without_FP1.vcf.format.filter.txt", header=TRUE, quote="", row.names=NULL)
...
annotation_table$AF_s<-annotation_table$AF4_a - annotation_table$AF3_a
...
#	geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2), data = lines)
	...
	ylim(-100,100)
print(g)
ggsave("FP2_without_FP1_delta.pdf",width=20,height=5)
...
#       geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2), data = lines)
		...
		ylim(0,100)
print(down)
ggsave("FP2_without_FP1_down.pdf",width=20,height=5)
up<-ggplot(annotation_table, aes(x=Pos/1000000, y=AF4_a*100, group=Chr))+
		...
		ylim(0,100)
print(up)
ggsave("FP2_without_FP1_up.pdf",width=20,height=5)
